...
p.t_T=FlightPathDataMat(:,1);
p.v_T=FlightPathDataMat(:,2);
p.phi_T=FlightPathDataMat(:,3);
p.gamma_T=FlightPathDataMat(:,4);
p.psi_T=FlightPathDataMat(:,5)+dpsi;

%% Simulation

r0_ME=[1; 1; 0]; % initial missile radius in earth coordinates [m]
v0_ME=[0; 0; -1000]; % initial missile velocity in earth coordinates [m/sec]
r0_TE=[-4540; 0; -10000*0.3048]; % initial target radius in earth coordinates [m]
