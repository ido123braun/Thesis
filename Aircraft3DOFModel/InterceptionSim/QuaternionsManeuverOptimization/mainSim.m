...
tfint = falcon.Parameter('IntermediateTime', 10, 0, 150, 1e-1);
tf = falcon.Parameter('FinalTime', 20, 0, 200, 1e-1);
...
% Add a Climb Phase
phase1 = problem.addNewPhase(@OptimalManeuver, x_vec, tau, 0, tfint);
phase1.addNewControlGrid(u_vec, tau);
phase1.Model.setModelOutputs(y_vec);...; falcon.Output('alpha'); falcon.Output('CL'); falcon.Output('CD'); falcon.Output('T'); falcon.Output('TSFC')]);

% Add a Maneuver Phase
phase2 = problem.addNewPhase(@OptimalManeuver, x_vec, tau, tfint, tf);
phase2.addNewControlGrid(u_vec, tau);
phase2.Model.setModelOutputs(y_vec);...; falcon.Output('alpha'); falcon.Output('CL'); falcon.Output('CD'); falcon.Output('T'); falcon.Output('TSFC')]);
...
% Set Intermediate Conditions

rfintmin_E=[-inf; -inf; -30000*0.3048]; % Minimum Terminal Airplane Radius in Earth Coordinates [m]
rfintmax_E=[inf; inf; -30000*0.3048]; % Maximum Terminal Airplane Radius in Earth Coordinates [m]
vfintmin=300; % Minimum Terminal Airplane TAS Velocity [m/sec]
vfintmax=300; % Maximum Terminal Airplane TAS Velocity [m/sec]
phifintmin=0*pi/180; % Minimum Terminal Airplane Roll Angle [rad]
phifintmax=0*pi/180; % Maximum Terminal Airplane Roll Angle [rad]
gammafintmin=0*pi/180; % Minimum Terminal Airplane Course Angle [rad]
gammafintmax=0*pi/180; % Maximum Terminal Airplane Course Angle [rad]
psifintmin=0*pi/180; % Minimum Terminal Airplane Azimuth Angle [rad]
psifintmax=0*pi/180; % Maximum Terminal Airplane Azimuth Angle [rad]
Wfintmin=20300*0.4536*p.g; % Minimum Terminal Airplane Weight [N]
Wfintmax=p.m0.*p.g; % Maximum Terminal Airplane Weight [N]
Pfintmin=0; % Minimum Terminal Airplane Roll Rate [rad/sec]
Pfintmax=0; % Maximum Terminal Airplane Roll Rate [rad/sec]
Jsfintmin=0; % Minimum Terminal Airplane Throttle
Jsfintmax=1; % Maximum Terminal Airplane Throttle
nfintmin=1; % Minimum Terminal Airplane Load Factor
nfintmax=1; % Maximum Terminal Airplane Load Factor

e0min=cos(psifintmin/2)*cos(gammafintmin/2)*cos(phifintmin/2)+sin(psifintmin/2)*sin(gammafintmin/2)*sin(phifintmin/2); % Minimum Terminal Quaternion Parameter 0
e1min=cos(psifintmin/2)*cos(gammafintmin/2)*sin(phifintmin/2)-sin(psifintmin/2)*sin(gammafintmin/2)*cos(phifintmin/2); % Minimum Terminal Quaternion Parameter 1
e2min=cos(psifintmin/2)*sin(gammafintmin/2)*cos(phifintmin/2)+sin(psifintmin/2)*cos(gammafintmin/2)*sin(phifintmin/2); % Minimum Terminal Quaternion Parameter 2
e3min=sin(psifintmin/2)*cos(gammafintmin/2)*cos(phifintmin/2)-cos(psifintmin/2)*sin(gammafintmin/2)*sin(phifintmin/2); % Minimum Terminal Quaternion Parameter 3
efintmin_P=[e0min; e1min; e2min; e3min]; % Minimum Terminal Quaternion Vector

e0max=cos(psifintmax/2)*cos(gammafintmax/2)*cos(phifintmax/2)+sin(psifintmax/2)*sin(gammafintmax/2)*sin(phifintmax/2); % Maximum Terminal Quaternion Parameter 0
e1max=cos(psifintmax/2)*cos(gammafintmax/2)*sin(phifintmax/2)-sin(psifintmax/2)*sin(gammafintmax/2)*cos(phifintmax/2); % Maximum Terminal Quaternion Parameter 1
e2max=cos(psifintmax/2)*sin(gammafintmax/2)*cos(phifintmax/2)+sin(psifintmax/2)*cos(gammafintmax/2)*sin(phifintmax/2); % Maximum Terminal Quaternion Parameter 2
e3max=sin(psifintmax/2)*cos(gammafintmax/2)*cos(phifintmax/2)-cos(psifintmax/2)*sin(gammafintmax/2)*sin(phifintmax/2); % Maximum Terminal Quaternion Parameter 3
efintmax_P=[e0max; e1max; e2max; e3max]; % Maximum Terminal Quaternion Vector
...
phase1.setInitialBoundaries([r0_E ; v0; e0_P; W0; P0; Js0; n0]);
phase1.setFinalBoundaries([rfintmin_E ; vfintmin; efintmin_P; Wfintmin; Pfintmin; Jsfintmin; nfintmin],[rfintmax_E ; vfintmax; efintmax_P; Wfintmax; Pfintmax; Jsfintmax; nfintmax]);
phase2.setFinalBoundaries([rfmin_E ; vfmin; efmin_P; Wfmin; Pfmin; Jsfmin; nfmin],[rfmax_E ; vfmax; efmax_P; Wfmax; Pfmax; Jsfmax; nfmax]);
...
problem.addNewParameterCost(tfint);
...
r_E=phase1.StateGrid.Values(1:3,:);
v=phase1.StateGrid.Values(4,:);
e_P=phase1.StateGrid.Values(5:8,:)';
W=phase1.StateGrid.Values(9,:);
P=phase1.StateGrid.Values(10,:);
Js=phase1.StateGrid.Values(11,:);
n=phase1.StateGrid.Values(12,:);

Pcom=phase1.ControlGrids.Values(1,:);
Jscom=phase1.ControlGrids.Values(2,:);
ncom=phase1.ControlGrids.Values(3,:);

phi=phase1.Model.ModelOutputGrid.Values(1,:);
gamma=phase1.Model.ModelOutputGrid.Values(2,:);
psi=phase1.Model.ModelOutputGrid.Values(3,:);
alpha=phase1.Model.ModelOutputGrid.Values(4,:);
CL=phase1.Model.ModelOutputGrid.Values(5,:);
CD=phase1.Model.ModelOutputGrid.Values(6,:);
T=phase1.Model.ModelOutputGrid.Values(7,:);
TSFC=phase1.Model.ModelOutputGrid.Values(8,:);
ng=phase1.Model.ModelOutputGrid.Values(9,:);
Wneg=phase1.Model.ModelOutputGrid.Values(10,:);
